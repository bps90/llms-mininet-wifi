...
- ap1 no canal 1, ap2 no canal 6 (canais não interferentes)
- APs conectados ao switch central s1
- Estações em posições distintas, simulando distribuição espacial
"""
...
def topology():
    ...
    # Estações Wi-Fi com IPs na mesma sub-rede e posições distintas
    sta1 = net.addStation('sta1', ip='10.0.0.1/24', position='10,20,0')
    sta2 = net.addStation('sta2', ip='10.0.0.2/24', position='20,25,0')
    sta3 = net.addStation('sta3', ip='10.0.0.3/24', position='40,20,0')
    sta4 = net.addStation('sta4', ip='10.0.0.4/24', position='50,25,0')

    # Pontos de acesso Wi-Fi com canais diferentes (não interferentes)
    ...
    # Aplica parâmetros de rádio às interfaces sem fio
    net.configureWifiNodes()

    info('*** Criando links Wi-Fi (associação estações <-> APs)\n')
    ...
    # Teste de desempenho TCP entre estações em APs diferentes
    net.iperf((sta1, sta4))
